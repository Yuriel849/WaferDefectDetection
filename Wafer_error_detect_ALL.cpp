...
vector<Rect> ContaminationDetect(vector<Rect>& find, cv::Mat& flawless_img, cv::Mat& search_img, cv::Mat& draw_img, cv::Mat& fill_img)
{
	...
	vector<Vec4i> hierarchy_con; //백터 hierarchy변수 생성
	...
	{	sub_img = search_img(find[k]).clone();
		...
		Mat src_open; //노이즈가 제거된 상태.
		int type = MorphTypes::MORPH_OPEN; // 노이즈를 제거하는 기능 MORPH_OPEN을 type에 담겠다.
		cv::morphologyEx(sub_img_draw, src_open, type, SE);// cv::morphologyEx(src_bin(입력), src_open(출력), type, SE);//morphologyEx 노이즈를 제거하겠다. 

		cv::findContours(src_open, contours_con, hierarchy_con, RETR_TREE, CHAIN_APPROX_SIMPLE);

		//cv::findContours(sub_img_draw, contours_con, hierarchy_con, RETR_TREE, CHAIN_APPROX_SIMPLE); //객체 범위값을 구하기 위해 객체 외곽선 검출
