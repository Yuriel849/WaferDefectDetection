...
void ContaminationDetect(vector<Rect>& find, cv::Mat& flawless_img, cv::Mat& search_img, cv::Mat& draw_img, cv::Mat& fill_img)
{
	...
	{
		rrt1 = minAreaRect(contours[k]);//객체 외곽선을 넘지 않을 만큼 의 사이즈를 wRrt에 담아줌
		rt1 = rrt1.boundingRect(); // 객체의 위치에 상관없이 정방향 사각형 생성

		if (13 <= rt1.width && rt1.width <= 17)//small
		{
			vRois_Small.push_back(rt1);
			cv::rectangle(draw_Pads, rrt1.boundingRect(), CV_RGB(0, 255, 0), 1); //src_openn에서 찾아낸 객체의 외곽선 부분의 wR2f.tl() 좌상 꼭지점과
		}
		if (18 <= rt1.width && rt1.width <= 22)//large
		{
			vRois_Large.push_back(rt1);
			cv::rectangle(draw_Pads, rrt1.boundingRect(), CV_RGB(0, 0, 255), 1); //src_openn에서 찾아낸 객체의 외곽선 부분의 wR2f.tl() 좌상 꼭지점과
		}
	}
	...
	for (size_t k = 0; k < contours.size(); k++) //contourss.size();의 크기만큼 for문을 돌린다.(새롭게 정의된 객체 사이즈(수) 
	{
		...
		rrt2 = minAreaRect(contours[k]);//객체 외곽선을 넘지 않을 만큼 의 사이즈를 wRrt에 담아줌
		rt2 = rrt2.boundingRect(); // 객체의 위치에 상관없이 정방향 사각형 생성

		if (160 <= rt2.width && rt2.width <= 180)
		{
			vRois.push_back(rt2);
			cv::rectangle(draw_img, rrt2.boundingRect(), CV_RGB(255, 0, 0), 1); //src_openn에서 찾아낸 객체의 외곽선 부분의 wR2f.tl() 좌상 꼭지점과
		}
	}
	...
	for (size_t k = 0; k < vRois.size(); k++) //contourss.size();의 크기만큼 for문을 돌린다.(새롭게 정의된 객체 사이즈(수) 
	{
		...
		for (size_t i = 0; i < vRois_Large.size(); i++)
		{
			...
			Mat Pad = sub_img(rt3).clone(); ///예외처리되지 않음 
			Mat Pad_Bin;
	...
	{
		cv::rectangle(draw_img, vRois_Large_Err[i], CV_RGB(255, 0, 255), 1); //src_openn에서 찾아낸 객체의 외곽선 부분의 wR2f.tl() 좌상 꼭지점과
	}
	for (size_t i = 0; i < vRois_Small_Err.size(); i++)
	{
		cv::rectangle(draw_img, vRois_Small_Err[i], CV_RGB(255, 0, 255), 1); //src_openn에서 찾아낸 객체의 외곽선 부분의 wR2f.tl() 좌상 꼭지점과
	}
}
